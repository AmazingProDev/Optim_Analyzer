    ...
    <StyleMap id="sm_${styleId}"><Pair><key>normal</key><styleUrl>#${styleId}_normal</styleUrl></Pair><Pair><key>highlight</key><styleUrl>#${styleId}_highlight</styleUrl></Pair></StyleMap>\n`
        ...
        <Style id="${lineStyleId}"><LineStyle><color>${kLineColor}</color><width>2</width></LineStyle><PolyStyle><fill>0</fill></PolyStyle></Style>\n`
            ...
            const genDesc = () => {
                ...
                let html = `<div style="font-family:'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; font-size:12px; width:450px; background:#1e1e1e; color:#e5e5e5; padding:0; border-radius:6px; overflow:hidden;">`;
            ...
            // Point Placemark
            pointGroups.get(groupName).push(`    <Placemark><name></name><description>${desc}</description><styleUrl>#sm_${styleId}</styleUrl>${geometryPoint}</Placemark>`);
        ...
        const buildFolders = (groupMap) => {
            ...
            orderedKeys.forEach(k => {
                // ADD ID TO FOLDER for linking? XML ID on Folder is valid.
                const safeId = 'folder_' + k.replace(/[^a-zA-Z0-9]/g, '');
                const visibilityTag = (k.toString().trim() === 'Connection Lines') ? '<visibility>0</visibility><open>0</open><styleUrl>#folder_style_hidden</styleUrl>\n' : '';
